...
router.put('/:id/cancelar', async (req, res) => {
    ...
    try {
        ...
        Number(reserva.id_estudiante) !== Number(req.session.usuario.id)        ) {
        ...
        if (["cancelada","rechazada"].includes(reserva.estado)) {
             return res.status(400).json({
        ok:false,
        mensaje:"La reserva ya no puede cancelarse."
    });
      ...
      const [vigencia] = await db.query(
    `SELECT
        TIMESTAMP(fecha, hora_inicio) > NOW() AS puede_cancelar
     FROM Reservas
     WHERE id_reserva = ?`,
    [req.params.id]
);
...
if (!vigencia[0]?.puede_cancelar) {
    return res.status(400).json({
        ok:false,
        mensaje:"La reserva ya comenzó o venció y no puede cancelarse."
    });
}
